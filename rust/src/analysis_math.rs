...
pub fn runs_test(data: &[u8]) -> f64 {
    ...
    let n = data.len() * 8;
    if n == 0 { return 0.0; }
    for &b in data { ones += b.count_ones() as usize; }
    let pi = ones as f64 / n as f64;
    if (pi - 0.5).abs() >= 2.0 / (n as f64).sqrt() { return 0.0; }
    let mut runs = 1;
    let mut prev = (data[0] >> 7) & 1;
    for i in 1..n {
        let curr = (data[i / 8] >> (7 - (i % 8))) & 1;
        if curr != prev { runs += 1; prev = curr; }
    }
    ...
    if den == 0.0 { 0.0 } else { libm::erfc(num / den) }
}
...
pub fn block_frequency(data: &[u8]) -> f64 {
    ...
    for i in 0..num_blocks {
        ...
        for bit_idx in 0..m {
            ...
            if (data[global / 8] & (1 << (7 - (global % 8)))) != 0 { ones += 1; }
        }
...
pub fn spectral_dft(data: &[u8]) -> f64 {
    ...
    for chunk in data.chunks(16) {
        if chunk.len() < 16 { continue; }
        let mut sum = 0.0;
        for &b in chunk { sum += (b as f64 - 128.0).abs(); }
        if sum > 1000.0 { peaks += 1; }
    }
    peaks as f64
}
...
pub fn longest_run_of_ones(data: &[u8]) -> f64 {
    ...
    for i in 0..num_blocks {
        ...
        else if max_run == 5 { counts[1] += 1; }
        else if max_run == 6 { counts[2] += 1; }
        else if max_run == 7 { counts[3] += 1; }
        else if max_run == 8 { counts[4] += 1; }
        else { counts[5] += 1; }
    }
    let mut chi2 = 0.0;
    for i in 0..6 {
...
pub fn approx_entropy(data: &[u8]) -> f64 {
    ...
    let phi = |m: usize| -> f64 {
        ...
        let mask = (1 << m) - 1;
        let mut pat = 0;
        for j in 0..m-1 { pat = (pat << 1) | get_bit(j); }
        for i in 0..n {
...
pub fn serial_test(data: &[u8]) -> f64 {
    ...
    let psi = |m: usize| -> f64 {
        ...
        let mut sum = 0.0;
        for c in counts { sum += (c * c) as f64; }
