...
void poly_decompress(poly *r, const uint8_t *a, int d) {
  if (d == 11) {
    for (int i = 0; i < 32; i++) {
      ...
      t[5] =
          (uint16_t)(a[6] >> 7) | ((uint16_t)a[7] << 1) | ((uint16_t)a[8] << 9);
      t[6] = (uint16_t)(a[8] >> 2) | ((uint16_t)a[9] << 6);
      t[7] = (uint16_t)(a[9] >> 5) | ((uint16_t)a[10] << 3);
      for (int j = 0; j < 8; j++) {
        r->coeffs[8 * i + j] =
            (int16_t)((((uint32_t)(t[j] & 0x7FF)) * Q + 1024) >> 11);
      }
      a += 11;
    }
  } else if (d == 5) {
    for (int i = 0; i < 32; i++) {
      ...
      for (int j = 0; j < 8; j++) {
        r->coeffs[8 * i + j] =
            (int16_t)((((uint32_t)(t[j] & 0x1F)) * Q + 16) >> 5);
      }
      a += 5;
    }
  }
...
/**
 * @brief Encodes a polynomial into a 384-byte array (3 bytes per 2
 * coefficients). Each coefficient is 12-bit (0-4095).
 */
...
void poly_frombytes(poly *r, const uint8_t *a) {
  for (int i = 0; i < 128; i++) {
    r->coeffs[2 * i + 0] =
        (int16_t)((a[3 * i + 0] >> 0) | ((uint16_t)a[3 * i + 1] << 8)) & 0xFFF;
    r->coeffs[2 * i + 1] =
...
int poly_getnoise_rej(poly *r, const uint8_t *seed, size_t seed_len) {
  ...
  while (count < 256) {
    ...
    for (int i = 0; i < SHAKE128_RATE && count < 256; i += 3) {
      ...
      uint16_t val1 =
          (uint16_t)((buf[i + 1] >> 4) | ((uint16_t)buf[i + 2] << 4)) & 0xFFF;
      if (val0 < Q)
        r->coeffs[count++] = (int16_t)val0;
      if (count < 256 && val1 < Q)
        r->coeffs[count++] = (int16_t)val1;
    }
  }
  return 256;
}
