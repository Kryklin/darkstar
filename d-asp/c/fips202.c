 ...
 */
static void keccak_absorb(uint64_t s[25], unsigned int rate, const uint8_t *in,
 ...
 */
static void keccak_squeeze(uint8_t *out, size_t outlen, uint64_t s[25],
                           unsigned int rate) {
...
void shake128_absorb(shake_ctx *state, const uint8_t *in, size_t inlen) {
  while (inlen > 0) {
    ...
    size_t chunk =
        (inlen < (size_t)(r - state->pos)) ? inlen : (size_t)(r - state->pos);
    for (size_t i = 0; i < chunk; i++) {
      state->s[(state->pos + i) / 8] ^= (uint64_t)in[i]
                                        << (8 * ((state->pos + i) % 8));
    }
...
void shake256_finalize(shake_ctx *state) { shake128_finalize(state); }
