 ...
 */
static void gen_matrix_expand(polyvec *a, const uint8_t seed[32],
                              int transposed) {
  unsigned int i, j;
  uint8_t extseed[34];
 ...
 */
static void crypto_pke_enc(uint8_t *ct, const uint8_t *m, const uint8_t *pk,
                           const uint8_t *coins) {
  ...
  for (i = 0; i < MLKEM_K; i++)
    poly_compress(ct + i * 352, &b.vec[i], 11);
...
DASP_EXPORT int crypto_kem_keypair(uint8_t *pk, uint8_t *sk) {
  ...
  // pk = encode(t) || rho
  for (i = 0; i < MLKEM_K; i++)
    poly_tobytes(pk + i * 384, &pkvec.vec[i]);
  memcpy(pk + MLKEM_POLYVECBYTES, rho, 32);

  // sk = encode(s) || pk || H(pk) || z
  for (i = 0; i < MLKEM_K; i++)
    poly_tobytes(sk + i * 384, &s.vec[i]);
...
DASP_EXPORT int crypto_kem_dec(uint8_t *ss, const uint8_t *ct,
  ...
  for (i = 0; i < CRYPTO_CIPHERTEXTBYTES; i++)
    fail1 |= (ct[i] ^ ct_prime[i]);

  uint32_t fail2 = 0;
  for (i = CRYPTO_CIPHERTEXTBYTES - 1; i >= 0; i--)
    fail2 |= (ct[i] ^ ct_prime[i]);
