...
(async () => {
  ...
  const results = [
    ...
    { name: 'Cargo (Rust)', status: 'Pending', vulns: 0, cmd: 'cd d-asp/rust && cargo audit --json', parse: parseCargo },
    { name: 'Govulncheck (Go)', status: 'Pending', vulns: 0, cmd: 'cd d-asp/go && govulncheck ./...', parse: parseGo },
