...
export async function runInteropBenchmark(
  ...
  await Promise.all(Object.entries(ENGINES).map(async ([name, engine]) => {
    ...
    if (lastOutput !== payload && status !== 'FAIL') {
      status = 'MISMATCH';
      console.error(`${name} Mismatch! Expected start: ${payload.substring(0, 50)}... Actual start: ${lastOutput.substring(0, 50)}... Lengths: Exp ${payload.length}, Act ${lastOutput.length}, lineCount: ${lineCount}`);
