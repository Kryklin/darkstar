...
class DarkstarCrypt:
    def __init__(self):
        ...
        wasm_path = os.path.join(
            os.path.dirname(__file__), "..", "wasm", "dasp_crypto.wasm"
        )
        ...
        getrandom_type = wasmtime.FuncType(
            [wasmtime.ValType.i32(), wasmtime.ValType.i32()], []
        )
        getrandom_func = wasmtime.Func(
        ...
        self.memory: Any = self.exports["memory"]
        self.wasm_alloc: Any = self.exports["wasm_alloc"]
        self.wasm_dealloc: Any = self.exports["wasm_dealloc"]
    ...
    def _read_string_from_wasm(self, ptr):
        ...
        while True:
            chunk = self.memory.read(
                self.store, ptr + offset, ptr + offset + CHUNK_SIZE
            )
            if b"\x00" in chunk:
