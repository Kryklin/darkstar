...
void hex_decode(const char *in, uint8_t *out, size_t out_len) {
...
int main(int argc, char **argv) {
    ...
    if (strcmp(argv[1], "decrypt") == 0) {
    ...
    } else if (strcmp(argv[1], "stream-decrypt") == 0) {
        if (argc < 3) return 1;
        char *sk_raw = argv[2];
        if (sk_raw[0] == '@') sk_raw = read_file(sk_raw + 1);
        uint8_t hwid[32];
        ...
        if (!line) { if (argv[2][0] == '@') free(sk_raw); return 1; }
        int num_streams = 2;
        cudaStream_t streams[2];
        for (int i = 0; i < num_streams; i++) CUDA_CHECK(cudaStreamCreate(&streams[i]));
        cudaEvent_t start_event, stop_event, t_start, t_stop;
        CUDA_CHECK(cudaEventCreate(&start_event));
        CUDA_CHECK(cudaEventCreate(&stop_event));
        ...
        uint8_t *h_payload;
        CUDA_CHECK(cudaMallocHost((void**)&h_payload, h_payload_cap));
        while (fgets(line, 1024 * 1024, stdin)) {
            CUDA_CHECK(cudaEventRecord(t_start, 0));
            ...
            size_t p_len = strlen(data_hex) / 2;
            if (p_len >= h_payload_cap) {
                CUDA_CHECK(cudaFreeHost(h_payload));
                h_payload_cap = p_len * 2;
                CUDA_CHECK(cudaMallocHost((void**)&h_payload, h_payload_cap));
            }
            hex_decode(data_hex, h_payload, p_len);
            uint8_t ct[CRYPTO_CIPHERTEXTBYTES];
            ...
            memcpy(final_plaintext, h_payload, p_len);
            final_plaintext[p_len] = '\0';
            CUDA_CHECK(cudaEventRecord(t_stop, 0));
        ...
        if (h_payload) CUDA_CHECK(cudaFreeHost(h_payload));
        CUDA_CHECK(cudaFree(d_payload));
        CUDA_CHECK(cudaFree(d_keys));
        CUDA_CHECK(cudaFree(d_nonce));
