 ...
 *
 * @param out_ct   Output buffer for the combined ciphertext (KEM CT + Payload +
 * MAC). Size: CRYPTO_CIPHERTEXTBYTES + payload_len + 32.
 ...
 * @return 0 on success.
 */
int dasp_encapsulate_data(unsigned char *out_ct, const unsigned char *pk,
 ...
 */
int dasp_decapsulate_data(unsigned char *out_payload, const unsigned char *sk,
                          const unsigned char *hwid, const unsigned char *in_ct,
                          size_t in_ct_len);

/* --- SPNA Engine Core Prototypes --- */

void spna_layer_substitution(unsigned char *state, size_t state_len,
                             const unsigned char *k_word);
...
void spna_layer_algebraic(unsigned char *state, size_t state_len,
                          const unsigned char *k_word);
