...
static void transform512(uint64_t *state, const uint8_t *data) {
  ...
  for (int i = 0; i < 16; i++) {
    w[i] =
        ((uint64_t)data[i * 8] << 56) | ((uint64_t)data[i * 8 + 1] << 48) |
...
void crypto_sha512(const uint8_t *data, size_t len, uint8_t *out) {
  ...
  for (int i = 0; i < 8; i++)
    buf[119 - i] = 0; // high 64 bits of bitlen (assume < 2^64)
  for (int i = 0; i < 8; i++)
    buf[127 - i] = (uint8_t)(bitlen >> (i * 8));
  transform512(state, buf);
  for (int i = 0; i < 8; i++) {
    for (int j = 0; j < 8; j++) {
...
void crypto_hmac_sha512(const uint8_t *key, size_t key_len, const uint8_t *data,
                        size_t data_len, uint8_t *out) {
  uint8_t k[128] = {0};
  uint8_t ipad[128], opad[128];
  if (key_len > 128)
    crypto_sha512(key, key_len, k);
  else
